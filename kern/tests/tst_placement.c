...
void _main(void)
{
	...
	cprintf_colored(TEXT_cyan, "\nSTEP 2: checking PLACEMENT...\n");
	{
		...
		cprintf_colored(TEXT_cyan, "	STEP A: checking PLACEMENT fault handling... [30%] \n");
		{
			...
			{
				is_correct = 0;
				cprintf_colored(TEXT_TESTERR_CLR, "PLACEMENT of stack page failed\n");
			}
			...
			if (arr[PAGE_SIZE * 1024 * 2] != 2)
			{
				is_correct = 0;
				cprintf_colored(TEXT_TESTERR_CLR, "PLACEMENT of stack page failed\n");
			}
			if (arr[PAGE_SIZE * 1024 * 2 + PAGE_SIZE] != 2)
			{
				is_correct = 0;
				cprintf_colored(TEXT_TESTERR_CLR, "PLACEMENT of stack page failed\n");
			}

			if (arr[PAGE_SIZE * 1024 * 3] != 3)
			{
				is_correct = 0;
				cprintf_colored(TEXT_TESTERR_CLR, "PLACEMENT of stack page failed\n");
			}
			if (arr[PAGE_SIZE * 1024 * 3 + PAGE_SIZE] != 3)
			{
				is_correct = 0;
				cprintf_colored(TEXT_TESTERR_CLR, "PLACEMENT of stack page failed\n");
			}
			...
			{
				is_correct = 0;
				cprintf_colored(TEXT_cyan, "new stack pages should NOT be written to Page File until evicted as victim\n");
			}
			...
			{
				is_correct = 0;
				cprintf_colored(TEXT_TESTERR_CLR, "allocated memory size incorrect. Expected Difference = %d, Actual = %d\n", expected, (freePages - sys_calculate_free_frames()));
			}
		}
		...
		cprintf_colored(TEXT_cyan, "	STEP B: checking WS entries... [30%]\n");
		{
			...
			{
				is_correct = 0;
				cprintf_colored(TEXT_TESTERR_CLR, "PAGE WS entry checking failed... trace it by printing page WS before & after fault\n");
			}
		}
		...
		cprintf_colored(TEXT_cyan, "	STEP C: checking working sets WHEN BECOMES FULL... [20%]\n");
		{
			...
			{
				is_correct = 0;
				cprintf_colored(TEXT_TESTERR_CLR, "wrong PAGE WS pointer location... trace it by printing page WS before & after fault\n");
			}
